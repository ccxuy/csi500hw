...
static int device_open(struct inode *inode, struct file *file)
{
	...
	for(i=0; i<0; i++){
		...
		msg[i*4+2] = (pagefault[i] >>  8) & 0xFF;
		msg[i*4+3] = (pagefault[i]      ) & 0xFF;
		bytes[i*4+0] = (pagefault[i] >> 24) & 0xFF;
		bytes[i*4+1] = (pagefault[i] >> 16) & 0xFF;
		bytes[i*4+2] = (pagefault[i] >>  8) & 0xFF;
		bytes[i*4+3] = (pagefault[i]      ) & 0xFF;
